...
describe('PgConnection', () => {
  ...
  let rollbackTransactioneSpy: jest.Mock
  ...
  it('should rollback transaction', async () => {
    ...
    expect(rollbackTransactioneSpy).toHaveBeenCalledWith()
    expect(rollbackTransactioneSpy).toHaveBeenCalledTimes(1)
  ...
  it('should return TransactionNotFoundError on rollback if queryRunner is not found', async () => {
    ...
    expect(rollbackTransactioneSpy).not.toHaveBeenCalled()
    await expect(promise).rejects.toThrow(new TransactionNotFoundError())
  })

  it('should get repository from transction', async () => {
    await sut.connect()
    await sut.openTransaction()
